...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every(({runtimeInSeconds}) => runtimeInSeconds > 120) ? true : false
}
...
function anySongIsOverFourMinutes(songs) {
  return songs.some(({ runtimeInSeconds }) => runtimeInSeconds > 240) ? true : false;
}
...
function anySongIsByPeanut(songs) {
  return songs.some(({artist}) => artist === "Peanut") ? true : false;
}
